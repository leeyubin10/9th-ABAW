...
class ConcordanceCorrelationCoefficient(object):
    ...
    def calculate_ccc(x, y):
        ...
        # ✅ NaN/Inf 방어
        if np.isnan(concordance_correlation_coefficient).any() or np.isinf(concordance_correlation_coefficient).any():
            concordance_correlation_coefficient = 0.0
...
class ContinuousMetricsCalculator(object):
    ...
    def calculator(self, output, label, metric):
        # # NaN/Inf 방어
        # if isinstance(output, np.ndarray) and isinstance(label, np.ndarray):
        #     if np.isnan(output).any() or np.isinf(output).any() or np.isnan(label).any() or np.isinf(label).any():
        #         print(f"❌ Skipping metric '{metric}' due to NaN or Inf in input.")
        #         if metric == "pcc":
        #             return (0.0, 1.0)  # pearsonr는 tuple 반환
        #         else:
        #             return 0.0
